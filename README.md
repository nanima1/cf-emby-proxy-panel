...
| 模块 | 能力 |
| --- | --- |
| 路由管理 | 用 `/hk`、`/home`、`/movie` 这类路径管理多个 Emby，支持拖拽排序、上移下移、JSON 导入导出 |
| 小白生成器 | 输入 Emby 原地址和入口路径，自动生成客户端要填写的反代后地址 |
| 外观背景 | 支持图床直链或随机图 API，自定义背景 URL 和透明度保存到 D1 `settings` 表 |
