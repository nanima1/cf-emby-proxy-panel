...
已经会用 Wrangler 的话，可以直接看：[快速部署](./QUICK_DEPLOY.md)
...
- 把上游 Emby 填成你的原地址。
- 显示反代后的客户端地址。
- 在路由表单里同步显示 `客户端入口预览`，保存前也能确认地址对不对。

确认无误后点 `保存路径`。

如果你想更省事，也可以直接点 `一键创建并复制`。它会自动保存这条路由，并把 `https://你的域名/hk` 复制到剪贴板，之后直接粘到 Emby 客户端里就行。

如果你填的入口路径已经存在，面板会先问你是否覆盖。看不懂时先点取消，再去路由列表里确认原来的配置。

保存成功后，路由卡片里会出现 `复制入口` 和 `复制配置`：

- `复制入口` 只复制 Emby 客户端要填的地址。
- `复制配置` 会复制路径、模式、浏览器访问策略、图片缓存状态和全部上游，适合备忘或发给别人帮你排查。
...
如果同一个路径下面有多个上游备用地址，可以在路由表单的 `上游 Emby` 里用逗号或换行分隔。第一个上游异常时，会自动尝试后面的上游。
...
| `8.8.8.8` | A 记录 |
| `2001:db8::1` | AAAA 记录 |
| `[2001:db8::1]` | 自动去掉中括号后创建 AAAA |
| `cdn.example.com` | CNAME 记录 |
| `https://cdn.example.com/path` | 自动取 `cdn.example.com` 创建 CNAME |

这些内容会被拒绝：

- 内网 IP，例如 `192.168.1.1`。
- 本机 IP，例如 `127.0.0.1`。
- 错误 IP，例如 `999.999.999.999`。
- 带空格或格式错误的域名。
